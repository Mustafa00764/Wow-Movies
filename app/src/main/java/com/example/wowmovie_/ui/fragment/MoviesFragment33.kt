...
class MoviesFragment33 : Fragment(R.layout.fragment_movies33) {
    lateinit var repository: MovieRepository
    var id = ""
val args: MoviesFragment33Args by navArgs()
    ...
    private fun initViews(view: View) {
        ...
        progresBar = view.findViewById(R.id.prgBarMovies33)
        ...
        ll = view.findViewById(R.id.ll_3)
        ivSave = view.findViewById(R.id.iv_Save33)
        ...
        val back1 = view.findViewById<ImageView>(R.id.iv_back222)
        back1.setOnClickListener {
            findNavController().navigate(R.id.action_moviesFragment33_to_upcomingMoviesFragment)
        }
        adapter.onClick={
            findNavController().navigate(R.id.action_moviesFragment33_self,
                bundleOf("moviesId" to it.toString())
            )
        }
    ...
    private fun fetchVideoMovie() {
        ...
        TMDBClient.api.getVideos(id.toInt()).enqueue(object :Callback<VideosUS>{
            override fun onResponse(call: Call<VideosUS>, response: Response<VideosUS>) {
                if (response.isSuccessful) response.body()?.results?.getOrNull(0)?.key.toString()
                    .also {videoKey = it } else{
                    Log.d("@@@@@@", "onResponse:${response.code()} ")
                    Log.d("@@@@@@", "onResponse:${response.errorBody()} ")
                }
            }
